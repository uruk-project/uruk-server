...
{
    public class EventReceiverServiceTests
    {
        ...
        {
            var service = new EventReceiverService(new TestEventSink(response: true));

            var response = await service.TryStoreToken(new ReadOnlySequence<byte>(ValidToken), TokenValidationPolicy.NoValidation);
        ...
        {
            var service = new EventReceiverService(new TestEventSink(response: false));
        ...
        public async Task TryStoreToken_InvalidRequest_ReturnsInvalidRequest()
        {
            ...
            var response = await service.TryStoreToken(new ReadOnlySequence<byte>(new byte[0]), policy);
        ...
        public async Task TryStoreToken_BadKey_ReturnsInvalidKey()
        {
            ...
            var response = await service.TryStoreToken(new ReadOnlySequence<byte>(ValidToken), policy);
